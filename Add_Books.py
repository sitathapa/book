...
class AddBooks:
    ...
    def add_gui(self):
        try:
            ...
            self.style.configure("mystyle.Treeview", font=('Ariel', 9, 'bold'))
            self.style.configure("mystyle.Treeview.Heading", font=('Ariel', 12, 'bold'))

            self.add_tree = ttk.Treeview(self.display, style="mystyle.Treeview", column=('n', 'w', 'p', 'm','c','e','r'), height=17)
            ...
            self. exit= Button(self.display, text='Delete',bg='black', fg='white',command=self.delete_item, font=('Ariel', 11, 'bold'))
            self.del_book.place(x=340, y=570)

            self.back1 = Button(self.display, text="Mainscreen",command=self.backbut, bg='yellow', font=('Ariel', 9, 'bold'))
            self.back1.place(x=1190, y=640)
